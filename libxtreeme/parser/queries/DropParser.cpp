...
namespace Xtreeme
{
	bool DropParser::readSilent(const Xtreeme::Token& t, bool& ok, bool& end)
	{
		if (t.code == QS("SILENT"))
		{
			ok = true;
	...
	MultiGraphSelector DropParser::readGraphSelector(const Xtreeme::Token& t, bool& ok, bool& end)
	{
		return selector.parse(t, ok, end);
	}

	Query* DropParser::parse(const Xtreeme::Token& t, bool& ok, bool& end)
	{
		bool subEnd = false;
